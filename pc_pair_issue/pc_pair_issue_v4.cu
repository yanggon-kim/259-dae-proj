...
__host__ __device__ __forceinline__ float producer_value_v4(
...
__host__ __device__ __forceinline__ float consumer_value_v4(int iters) {
    float a0 = 0.125000f;
    float a1 = 0.250000f;
    float a2 = 0.375000f;
    float a3 = 0.500000f;
...
extern "C" __global__ void pc_pair_v4_baseline_kernel(
    const float* __restrict__ a, const float* __restrict__ b,
    const int* __restrict__ index, float* __restrict__ producer_out,
    float* __restrict__ consumer_out, int n, int iters) {
    int i = blockIdx.x * blockDim.x + threadIdx.x;
    if (i >= n) return;
    producer_out[i] = producer_value_v4(a, b, index, i, n, iters);
    consumer_out[i] = consumer_value_v4(iters);
}

extern "C" __global__ __launch_bounds__(kBlockThreads, 1)
void pc_pair_v4_split_kernel(const float* __restrict__ a,
    ...
    if (threadIdx.x < kProducerThreads) {
        ...
        float value = consumer_value_v4(iters);
        unsigned zero_dep;
...
void reference(const std::vector<float>& a, const std::vector<float>& b,
    ...
    for (int i = 0; i < n; ++i) {
        producer_ref[i] =
            producer_value_v4(a.data(), b.data(), index.data(), i, n, iters);
        consumer_ref[i] = consumer_value_v4(iters);
    }
}
...
int main(int argc, char** argv) {
    ...
    auto launch = [&]() {
        if (split) {
            pc_pair_v4_split_kernel<<<grid, block>>>(da, db, dindex, dproducer,
                                                     dconsumer, opt.n,
                                                     opt.iters);
        } else {
            pc_pair_v4_baseline_kernel<<<grid, block>>>(
