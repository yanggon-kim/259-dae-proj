...
__host__ __device__ __forceinline__ float producer_value_v2(
...
__host__ __device__ __forceinline__ float consumer_value_v2(
    const float* __restrict__ c, const float* __restrict__ d, int i, int n,
    int iters) {
    float a0 = c[i];
    float a1 = c[wrap_index(i + 53, n)];
    float a2 = c[wrap_index(i + 107, n)];
    float a3 = c[wrap_index(i + 211, n)];
    float b0 = d[i];
    float b1 = d[wrap_index(i + 59, n)];
    float b2 = d[wrap_index(i + 113, n)];
    float b3 = d[wrap_index(i + 223, n)];

#pragma unroll 4
    for (int r = 0; r < iters; ++r) {
        a0 = fmaf(a0, 1.00013f, b0);
        a1 = fmaf(a1, 0.99991f, b1);
        a2 = fmaf(a2, 1.00031f, b2);
        a3 = fmaf(a3, 0.99973f, b3);
        b0 = fmaf(b0, 0.99987f, 0.03125f);
        b1 = fmaf(b1, 1.00007f, 0.06250f);
        b2 = fmaf(b2, 0.99961f, 0.09375f);
        b3 = fmaf(b3, 1.00019f, 0.12500f);
    }

    return (a0 + a1) + (a2 + a3) + 0.25f * ((b0 + b1) + (b2 + b3));
}

extern "C" __global__ void pc_pair_v2_baseline_kernel(
    const float* __restrict__ a, const float* __restrict__ b,
    ...
    if (i >= n) return;
    producer_out[i] = producer_value_v2(a, b, index, i, n, iters);
    consumer_out[i] = consumer_value_v2(c, d, i, n, iters);
}

extern "C" __global__ __launch_bounds__(kBlockThreads, 1)
void pc_pair_v2_split_kernel(const float* __restrict__ a,
...
void initialize(std::vector<float>& a, std::vector<float>& b,
    ...
    for (int i = 0; i < n; ++i) {
        ...
        c[i] = 0.625f + static_cast<float>(i % 8192) * 0.00011f;
        d[i] = -0.875f + static_cast<float>(i % 16384) * 0.00007f;
        index[i] = (i * 67 + 29) % n;
    }
}
...
void reference(const std::vector<float>& a, const std::vector<float>& b,
    ...
    for (int i = 0; i < n; ++i) {
        producer_ref[i] =
            producer_value_v2(a.data(), b.data(), index.data(), i, n, iters);
        consumer_ref[i] = consumer_value_v2(c.data(), d.data(), i, n, iters);
    }
}
...
int main(int argc, char** argv) {
    ...
    auto launch = [&]() {
        if (split) {
            pc_pair_v2_split_kernel<<<grid, block>>>(da, db, dc, dd, dindex,
                                                     dproducer, dconsumer,
                                                     opt.n, opt.iters);
        } else {
            pc_pair_v2_baseline_kernel<<<grid, block>>>(
                da, db, dc, dd, dindex, dproducer, dconsumer, opt.n,
                opt.iters);
        }
    };
